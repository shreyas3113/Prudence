export const aiModels = {
    "llama4-maverick-17b-128e-instruct": {
        name: "Llama 4 Maverick 17B 128E Instruct",
        icon: "./assets/images/llama.svg",
        description: "Llama 4 Maverick 17B 128E Instruct model",
        responses: ["Processing with Llama 4 Maverick 17B 128E Instruct...", "Here's my response from Llama 4 Maverick 17B 128E Instruct."]
    },
    "llama4-scout-17b-16e-instruct": {
        name: "Llama 4 Scout 17B 16E Instruct",
        icon: "./assets/images/llama.svg",
        description: "Llama 4 Scout 17B 16E Instruct model",
        responses: ["Processing with Llama 4 Scout 17B 16E Instruct...", "Here's my response from Llama 4 Scout 17B 16E Instruct."]
    },
    "llama3-8b": {
        name: "Llama 3.1 8B",
        icon: "./assets/images/llama.svg",
        description: "Llama 3.1 8B model",
        responses: ["Processing with Llama 3.1 8B...", "Here's my response from Llama 3.1 8B."]
    },
    "llama3-70b": {
        name: "Llama 3.3 70B",
        icon: "./assets/images/llama.svg",
        description: "Llama 3.3 70B model",
        responses: ["Processing with Llama 3.3 70B...", "Here's my response from Llama 3.3 70B."]
    },
    "qwen-3-32b": {
        name: "Qwen 3 32B",
        icon: "./assets/images/dragon.svg",
        description: "Qwen 3 32B model",
        responses: ["Processing with Qwen 3 32B...", "Here's my response from Qwen 3 32B."]
    },
    "qwen-3-235b-a22b": {
        name: "Qwen 3 235B A22B",
        icon: "./assets/images/dragon.svg",
        description: "Qwen 3 235B A22B model",
        responses: ["Processing with Qwen 3 235B A22B...", "Here's my response from Qwen 3 235B A22B."]
    },
    "gemini-2.5-flash": { 
        name: "Gemini 2.5 Flash", 
        icon: "✨", 
        description: "Latest fast and efficient Google AI model", 
        responses: ["Here's a creative take!", "Imagination is key. Let me think...", "Let's explore that idea."],
        api: "gemini"
    },
    "gemini-2.0-flash": { 
        name: "Gemini 2.0 Flash", 
        icon: "🚀", 
        description: "Fast and reliable Google AI model for everyday tasks", 
        responses: ["Let me analyze this thoroughly.", "Here's a comprehensive response.", "I'll provide detailed insights."],
        api: "gemini"
    },
    "gpt-4": { 
        name: "GPT-4", 
        icon: "🤖", 
        description: "Advanced reasoning and analysis", 
        responses: ["Sure, how can I help you?", "Let me look that up for you.", "That's a great question!"] 
    },
    "claude": { 
        name: "Claude", 
        icon: "🧠", 
        description: "Helpful and balanced responses", 
        responses: ["I'd be happy to assist!", "How can I support you today?", "Let's solve this together."] 
    },
    
    
    
    
   
    "perplexity": { 
        name: "Perplexity", 
        icon: "📚", 
        description: "Real-time web information", 
        responses: ["Here's what I found.", "Fetching info from the web...", "Let me provide the most recent data."] 
    },
    "copilot": { 
        name: "Copilot", 
        icon: "👨‍💻", 
        description: "Your coding partner", 
        responses: ["Let's code it out!", "Try this snippet.", "Here's a quick fix."] 
    },
    "bard": { 
        name: "Bard", 
        icon: "🎤", 
        description: "Google's creative assistant", 
        responses: ["Let's get poetic.", "Here's a story you might enjoy.", "Creating something special…"] 
    },
    "llama": { 
        name: "LLaMA", 
        icon: "🦙", 
        description: "Meta's open model", 
        responses: ["Analyzing from my knowledge base.", "That's interesting!", "Let's dive in."] 
    },
    "mistral": { 
        name: "Mistral", 
        icon: "🐉", 
        description: "Lightweight performant AI", 
        responses: ["Fast and efficient — here's your answer.", "No delays, just solutions.", "Responding swiftly."] 
    }
}; 